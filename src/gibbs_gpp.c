...
void GIBBS_sumpred_gpp(int *aggtype, int *cov, int *spdecay, double *flag, 
     ...
     double *rho, double *mu_l, double *X, double *z, double *ObsZ, double *w0, double *w,
     int *nsite, int *nsiterT, double *dnsm, double *Xpred, int *transform, 
     double *accept_f, double *gof, double *penalty, int *thin)    
{
//     unsigned iseed = 44;
//     srand(iseed); 
     
     int its1, col, i, j, m1, r1, rT1, p1, N1, rep1, nsite1, brin, trans1, thin1;
     its1 = *its;
     col = *constant;
//     n1 = *n;
     m1 = *m;
...
//////////// other files
     FILE *fittedfile;
     fittedfile = fopen("OutGPP_Values_FittedValue.txt", "w");
     FILE *predfittedfile;
     predfittedfile = fopen("OutGPP_Values_Pred_FittedValue.txt", "w");
     FILE *latentfile;
     latentfile = fopen("OutGPP_Values_LatentValue.txt", "w");
/////////////////////////
...
*/
     ...
     wp, sig2ep, betap, Xpred, constant, zp, zfp);
    ...
    // for pmcc, fitted sum values    
         if(i >= brin){  
            for(j=0; j < N1; j++){
              ...
              if(flag[j]==0.0) fprintf(fittedfile, "%f ",zfit[j]);
            }
            fprintf(fittedfile,"\n");
         }
    ...
    // prediction samples
         ...
         for(j=0; j<(nsite1*rT1); j++){
             if(trans1 == 0){
               ...
               fprintf(predfittedfile, "%f ", zfp[j]);
             }
             if(trans1 == 1){
               zp[j] = zp[j]*zp[j];
               fprintf(predfile, "%f ", zp[j]);
...
// all parameters updated
      // add in loop for imputing latent varaible and poisson distribution
      if(i%thin1==0){
          for(int ii=0; ii<N1; ii++){
            if(flag[ii]==0.0) {
              double z_cand = rnorm(z[ii],latent_tune);
              double ll_cand = ObsZ[ii]*z_cand - exp(z_cand) + dnorm(z_cand,zfit[ii],sqrt(sig2e1[0]),1);
              double ll_curr = ObsZ[ii]*z[ii] - exp(z[ii]) +dnorm(z[ii],zfit[ii],sqrt(sig2e1[0]),1);
              double MRrat = exp(ll_cand-ll_curr);
    //Rprintf(" ObsZ = %3.2f, z = %3.2f, z_cand = %3.2f. ll_cand = %3.2f. ll_curr = %3.2f\n",
    //      ObsZ[ii], z[ii], z_cand, ll_cand, ll_curr); 
              if (runif(0,1)<MRrat) {
                z[ii] = z_cand;
                latentAccept++;
              }
              fprintf(latentfile, "%f ",z[ii]);
            }
          }
          fprintf(latentfile, "\n");
    ...
    //      ObsZ[ii], z[ii], z_cand, ll_cand, ll_curr); 
      ...
      if(i<4000) { // hard coded tuning
        if(i%100 == 0) {
          double acceptRate = ((double) latentAccept/(100.0*(N1-miss)))/0.4;
          Rprintf(" N1 = %d. miss = %d. latentAccept = %d. latent acceptRate = %3.2f. tuning value = %3.2f\n",N1, miss, latentAccept,acceptRate*0.4,latent_tune);
          if((acceptRate<0.9) || (acceptRate>1.1)) {
             if(acceptRate<0.5) acceptRate = 0.5;
             if(acceptRate>2.0) acceptRate = 2.0;
             latent_tune *= acceptRate;
          }
          latentAccept = 0;
          
        }
        
      } // end latent variable loop
...
//     iit = its1 - brin;
	...
	double *iit;
    iit = (double *) malloc((size_t)((col)*sizeof(double)));
	iit[0] = (its[0] - burnin[0])/ (double) thin1;
...
void GIBBSsp_zfitsum_onephi_gpp(int *intercept, int *cov, int *spdecay, 
     ...
     double *Xsp, double *z, double *ObsZ, double *w0, double *w, int *transform, 
