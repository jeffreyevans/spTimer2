...
void z_pr_its_gpp1_sp(int *cov, int *scale, int *its, int *nsite, int *n, 
     ...
     double *zpred)
{
     int i, j, its1, nsite1, m1, rT1, p1, q1, col,r1;
     its1 = *its;
     nsite1 = *nsite;
     m1 = *m;
     r1 =*r;
     ...
     w = (double *) malloc((size_t)((r1*m1)*sizeof(double)));
     ...
     int *constant, double *betapred, double *zpred)
{
     ...
     Aw = (double *) malloc((size_t)((r1*nsite1)*sizeof(double)));
     tAw = (double *) malloc((size_t)((r1*nsite1)*sizeof(double)));
...
// wp is a m x r matrix
     MProd(wp, r, m, A, nsite, Aw);  // nsite x r  
     MTranspose(Aw, r, nsite, tAw); // r x nsite
...
*/
     ...
     for(l=0; l < r1; l++) {
       for(t=0; t < T1; t++) {             
         ...
         for(i=0; i < nsite1; i++){
            int temp = i+t*nsite1+l*nsite1*T1;
            XBf[temp] = XBsp[i];
            mvrnormal(constant, mu, sig2ep, constant, out);
            zpred[temp] = XB[temp] + tAw[i*r1+l] + XBf[temp] + out[0];
         }
       } // end of t
     } // end of l
...
{
     int i, j, its1, nsite1, m1, rT1, p1, col, r1;
     its1 = *its;
     nsite1 = *nsite;
     m1 = *m;
     r1 =*r;
...
{
     int i,t,l, m1, nsite1, rT1, col, r1;
...
// wp is a m x rT matrix
     ...
     for(l=0; l<r1; l++){
       for(t=0; t<T1[l]; t++) {
         for(i=0; i< nsite1; i++){
            int temp=i+t*nsite1+nsite1*T2[l];
            mvrnormal(constant, mu, sig2ep, constant, out);
            zpred[temp] = XB[temp] + tAw[i*r1+l] + out[0];
         }
       }
     }
                            
     free(S_eta); free(det); free(Snsm); free(A);free(T1);free(T2); 
