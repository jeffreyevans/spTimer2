...
//     col = *constant;
   ...
   Aw = (double *) malloc((size_t)((n1*r1)*sizeof(double)));   
...
Rprintf("spatial decay set up\n");

   MProd(wp, r, m, A, n, Aw);  // n x r
Rprintf("Mprod done\n");
   rho_gpp(m, r, T, rT, p, mu_rho, delta2_rho, Qeta, w0p, wp, constant, rhop);        
Rprintf("rho_gpp done\n");
   beta_gpp(n, p, r, T, rT, N, mu_beta, delta2_beta, sig2e, X, Aw, z, 
   constant, betap); 
Rprintf("beta_gpp done\n");
...
//   mu_l_gpp(m, r, sig2l, alpha_l, delta2_l, Sinv, w0p, constant, mu_lp);         
   ...
   constant, sig2etap);
   sig_e_gpp(n, r, T, rT, N, shape_e, prior_b, XB, Aw, z, constant, sig2ep);
   Z_fit_gpp(flag, n, m, T, r, rT, sig2ep, Aw, XB, z, constant, zfit);
Rprintf("about to free variables\n");
...
//     col = *constant;
   ...
   A = (double *) malloc((size_t)((n1*m1)*sizeof(double)));   
   C = (double *) malloc((size_t)((n1*m1)*sizeof(double)));   
   Aw = (double *) malloc((size_t)((r1*n1)*sizeof(double)));   
...
//     exit(9);
   ...
   if(*intercept == 0){
     ...
     beta_gpp_for_sp(n, p, r, T, rT, N, mu_beta, delta2_beta, sig2e, X, XBsp, Aw, z, 
...
void sig_e_gpp(int *n, int *r, int *T, int *rT, int *N, double *shape, double *prior_b, 
     double *XB, double *Aw, double *z, int *constant, double *sig2e)
{
     int i,j,k,l,t, col, N1,n1,r1,rT1;
     n1 =*n;
     col =*constant;
     N1 =*N;
     r1 = *r;
     ...
     double *tAw, *er, *out;
     tAw = (double *) malloc((size_t)((n1*r1*col)*sizeof(double)));
     ...
     MTranspose(Aw, r, n, tAw); // r x n
     for(l=0; l<r1;l++){
       for(t=0;t<T1[l];t++){
         for(i=0; i<n1; i++){
            int temp = i+t*n1+n1*T2[l];
            er[temp] = (z[temp] -XB[temp] -tAw[i*r1+l]);
         }
       }
     }
     ...
     free(tAw); free(er); free(out),free(T1);free(T2);
     return;                  
       ...
       for(l=0; l<r1; l++){
          for(t=0; t<T1; t++){
           ...
           extract_alt2(l, t, n, rT, T, XBrest, XBrest1); // n x 1        
             for(i=0; i<n1; i++){
                z1[i] = z1[i]-XBno1[i]-XBrest1[i]-Aw[i+l*n1];;
             }
           }
           else {
             for(i=0; i<n1; i++){
                z1[i] = z1[i]-XBno1[i]-Aw[i+l*n1];;
             }
           }   
...
void beta_gpp(int *n, int *p, int *r, int *T, int *rT, int *N, double *mu_beta, 
     double *delta2_beta, double *sig2e, double *X, double *Aw, 
     double *z, int *constant, double *beta) 
{
     int i,l,t, p1, N1, col, r1,n1,rT1;
     p1 =*p;
     N1 =*N;
     col =*constant;
     r1 = *r;
...
//Rprintf("variables defined\n");
     ...
     MTranspose(Aw, r, n, tAw);  // r x n
//Rprintf("about to begin loop\n");
     for(l=0; l<r1;l++){
//Rprintf("r=%d, T[l] = %d, n1 =%d\n",r1,T1[l],n1);
       for(t=0;t<T1[l];t++){
         for(i=0; i<n1; i++){
            int temp = i+t*n1+n1*T2[l];
            zA[temp] = z[temp] - tAw[i*r1+l];
         }
       }
     }
...
//Rprintf("about to free and leave\n");

     free(delta2); free(det); free(tX); free(XX); free(md);
     free(tAw); free(zA); free(Xz); free(mean1);free(T1);free(T2);
...
void beta_gpp_for_sp(int *n, int *p, int *r, int *T, int *rT, int *N, double *mu_beta, 
     ...
     double *z, int *constant, double *beta) 
{
     ...
     XX = (double *) malloc((size_t)((p1*p1)*sizeof(double)));
     md = (double *) malloc((size_t)((p1*col)*sizeof(double)));
     tAw = (double *) malloc((size_t)((r1*n1*col)*sizeof(double)));
     zA = (double *) malloc((size_t)((N1*col)*sizeof(double)));
     Xz = (double *) malloc((size_t)((p1*col)*sizeof(double)));
     mean1 = (double *) malloc((size_t)((p1*col)*sizeof(double)));
     ...
     for(l=0; l<r1;l++){
       for(t=0;t<T1[l];t++){
         for(i=0; i<n1; i++){
            zA[i*rT1+T2[l]+t] = z[i*rT1+T2[l]+t] - tAw[i*r1+l];
         }
       }
     }
     MProd(zA, constant, N, tX, p, Xz);
     ...
     double *Sinv_0, double *w0, int *constant, double *mu_l)         
{
     ...
     for(i=0; i <m1; i++){
       ...
       del[0] = del[0]/sig2l[0] + 1.0/delta2_l[0];
       del[0] = 1.0/del[0];
       for(i=0; i<m1; i++){         
          w[i] = w0[i];         
       }
       ...
       chi[0] = chi[0]/sig2l[0] + alpha_l[l]/delta2_l[0];
...
{     
     ...
     for(i=0; i<m1*m1; i++){
         del[i] = Q_eta[i]*rho[0]*rho[0]+Sinv_0[i]/sig2l[0];
     }
     MInv(del, del, m, det);
     ...
     for(i=0; i<m1; i++){
        chi[i]=pt1[i]*rho[0]+pt2[i]*(mu_l[0]/sig2l[0]);
     }
